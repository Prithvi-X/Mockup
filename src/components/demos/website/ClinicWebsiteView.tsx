    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 selection:bg-neutral-800">
      ...
      <nav className="border-b border-neutral-900 bg-neutral-950/80 backdrop-blur px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6 text-xs text-neutral-400">
          <a href="#doctors" className="hover:text-white transition">Our Doctors</a>
          <a href="#specialties" className="hover:text-white transition">Specialties & Diagnostics</a>
          <a href="#reviews" className="hover:text-white transition">Patient Testimonials</a>
          <a href="#contact" className="hover:text-white transition">OPD Timings & Contact</a>
        </div>
      ...
      <section className="relative px-6 py-16 sm:py-24 max-w-5xl mx-auto text-center">
        ...
        <div className="mt-12 grid grid-cols-2 sm:grid-cols-4 gap-4 text-left border-y border-neutral-900 py-6">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-xs font-semibold text-white">Live Token Queue</div>
              <div className="text-[11px] text-neutral-400">Zero waiting room crowd</div>
            </div>
          </div>
          <div className="flex items-center gap-2.5">
            <FileText className="w-5 h-5 text-indigo-400 shrink-0" />
            <div>
              <div className="text-xs font-semibold text-white">Digital WhatsApp Rx</div>
              <div className="text-[11px] text-neutral-400">Never lose medical records</div>
            </div>
          </div>
          <div className="flex items-center gap-2.5">
            <ShieldCheck className="w-5 h-5 text-emerald-400 shrink-0" />
            <div>
              <div className="text-xs font-semibold text-white">NABH Hygiene Protocols</div>
              <div className="text-[11px] text-neutral-400">100% sanitized clinics</div>
            </div>
          </div>
          <div className="flex items-center gap-2.5">
            <Star className="w-5 h-5 text-amber-400 shrink-0" />
            <div>
              <div className="text-xs font-semibold text-white">4.9 ★ Patient Trust</div>
              <div className="text-[11px] text-neutral-400">Over 3,400+ consultations</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Doctors Roster */}
      <section id="doctors" className="px-6 py-12 max-w-5xl mx-auto border-t border-neutral-900">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-4">
          <div>
            <span className="text-xs font-semibold uppercase tracking-wider text-neutral-400">Senior Medical Staff</span>
            <h3 className="text-2xl font-bold text-white mt-1">Our Doctors & Consultants</h3>
          </div>
          <button
            type="button"
            onClick={() => setViewMode('customer')}
            ...
            >
              <div>
                <div className="flex items-center gap-3 mb-3">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-base ${themeClasses.accentBadge}`}>
                    {doc.name.replace('Dr. ', '').substring(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <h4 className="font-semibold text-white text-sm">{doc.name}</h4>
                    <p className="text-[11px] text-neutral-400">{doc.degree}</p>
                  </div>
                </div>

                <div className="mt-2 text-xs text-neutral-300 font-medium">
                  {doc.specialty}
                </div>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-neutral-800/80 flex items-center justify-between">
                <div>
                  <div className="text-[10px] text-neutral-400">Consultation Fee</div>
                  <div className="text-lg font-bold text-white">₹{doc.fee}</div>
                </div>
                <button
                  type="button"
                  onClick={() => setViewMode('customer')}
      ...
      {/* Specialties */}
      <section id="specialties" className="px-6 py-12 max-w-5xl mx-auto border-t border-neutral-900">
        <div className="text-center max-w-xl mx-auto mb-8">
          <span className="text-xs font-semibold uppercase tracking-wider text-neutral-400">Complete Care</span>
          <h3 className="text-2xl font-bold text-white mt-1">Specialties & Diagnostics</h3>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
          <div className="p-4 rounded-xl bg-neutral-900/30 border border-neutral-800 flex flex-col items-center">
            ...
            <h5 className="text-xs font-semibold text-white">General Medicine</h5>
            <p className="text-[11px] text-neutral-400 mt-0.5">Fever, diabetes, hypertension</p>
          </div>
          <div className="p-4 rounded-xl bg-neutral-900/30 border border-neutral-800 flex flex-col items-center">
            <Sparkles className="w-6 h-6 text-indigo-400 mb-2" />
            <h5 className="text-xs font-semibold text-white">Dental & Implants</h5>
            <p className="text-[11px] text-neutral-400 mt-0.5">Painless root canal & scaling</p>
          </div>
          <div className="p-4 rounded-xl bg-neutral-900/30 border border-neutral-800 flex flex-col items-center">
            <HeartPulse className="w-6 h-6 text-rose-400 mb-2" />
            <h5 className="text-xs font-semibold text-white">Orthopedic & Joint Care</h5>
            <p className="text-[11px] text-neutral-400 mt-0.5">Arthritis, fractures & rehab</p>
          </div>
          <div className="p-4 rounded-xl bg-neutral-900/30 border border-neutral-800 flex flex-col items-center">
            <FileText className="w-6 h-6 text-emerald-400 mb-2" />
            <h5 className="text-xs font-semibold text-white">In-House Lab Tests</h5>
            <p className="text-[11px] text-neutral-400 mt-0.5">Blood reports in 2 hours</p>
          </div>
        </div>
      </section>
      ...
      <footer id="contact" className="border-t border-neutral-800 bg-neutral-950 px-6 py-12">
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 text-xs">
          ...
          <div>
            <h4 className="font-semibold text-white text-sm mb-2">OPD Consultation Hours</h4>
          ...
          <div>
            <h4 className="font-semibold text-white text-sm mb-2">Patient Reception</h4>
        ...
        <div className="max-w-5xl mx-auto mt-8 pt-4 border-t border-neutral-900 text-center text-neutral-400 text-[11px] flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <span className="text-neutral-400">Powered by ATMAN Healthcare Token & E-Prescription System</span>
        </div>
      </footer>
    </div>
  );
